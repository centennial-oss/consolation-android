...
int ConvertPipeline::handle_frame(uvc_frame_t *frame) {
	...
	if (next_pipeline) {
		if UNLIKELY(!mFrameConvFunc) {
			next_pipeline->queueFrame(frame);
			RETURN(1, int);
		}
		uvc_frame_t *converted = get_frame(frame->actual_bytes);
		if UNLIKELY(!converted) {
			LOGW("convert: pool empty, pass-through queue");
			next_pipeline->queueFrame(frame);
			RETURN(1, int);
		}
		const uvc_error_t conversion_err = mFrameConvFunc(frame, converted);
		if UNLIKELY(conversion_err) {
			LOGW("failed to convert:%d", conversion_err);
			recycle_frame(converted);
			next_pipeline->queueFrame(frame);
			RETURN(1, int);
		}
		const int route = next_pipeline->tryEnqueueOwning(converted);
		if LIKELY(route == UVC_SUCCESS)
			RETURN(1, int);
		next_pipeline->queueFrame(converted);
		recycle_frame(converted);
	}
...
int register_convert_pipeline(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"org/centennialoss/consolation/uvc/ConvertPipeline",
