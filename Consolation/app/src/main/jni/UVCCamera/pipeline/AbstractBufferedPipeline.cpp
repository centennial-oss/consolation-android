...
int AbstractBufferedPipeline::queueOwningFrame(uvc_frame_t *owned_frame) {
	ENTER();
	int ret = UVC_ERROR_OTHER;
	if LIKELY(owned_frame)
		ret = add_frame(owned_frame);
	RETURN(ret, int);
}

int AbstractBufferedPipeline::tryEnqueueOwning(uvc_frame_t *owned_exclusive) {

	ENTER();
	if UNLIKELY(!owned_exclusive)
		RETURN(UVC_ERROR_INVALID_PARAM, int);
	const int enqueue_code = queueOwningFrame(owned_exclusive);
	RETURN(enqueue_code, int);
}
...
uvc_frame_t *AbstractBufferedPipeline::get_frame(const size_t &data_bytes) {
	...
	if (UNLIKELY(frame_pool.empty() && (total_frame_num < max_buffer_num))) {
		...
		n -= total_frame_num;
		if (LIKELY(n > 0)) {
			for (uint32_t grow_ix = 0; grow_ix < n; grow_ix++) {
				uvc_frame_t *new_slot = uvc_allocate_frame(data_bytes);
				if (LIKELY(new_slot)) {
					frame_pool.push_back(new_slot);
					total_frame_num++;
				} else {
					LOGW("get_frame: pool grow alloc failed at %u/%u", grow_ix, n);
					break;
				}
			}
			LOGW("allocate new frame(s), total_frame_num:%d", total_frame_num);
		} else {
...
void AbstractBufferedPipeline::init_pool(const size_t &data_bytes) {
	...
	pool_mutex.lock();
	{
		...
		const size_t frame_sz = data_bytes ? data_bytes : DEFAULT_FRAME_SZ;
...
int AbstractBufferedPipeline::add_frame(uvc_frame_t *frame) {
	...
	buffer_mutex.lock();
	{
		while (isRunning() && frame_buffers.size() >= max_buffer_num &&
			!frame_buffers.empty()) {
			uvc_frame_t *tail = frame_buffers.front();
...
uvc_frame_t *AbstractBufferedPipeline::wait_frame() {
	...
	while (isRunning() && frame_buffers.empty())
		buffer_sync.wait(buffer_mutex);
	if (LIKELY(isRunning() && !frame_buffers.empty())) {
