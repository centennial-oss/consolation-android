...
int DistributePipeline::handle_frame(uvc_frame_t *frame) {
	...
	Mutex::Autolock lock(pipeline_mutex);
	/*
	 * Each downstream must receive an independent duplicate (parallel handlers).
	 * tryEnqueueOwning from one decoded buffer cannot fan out safely without
	 * refcounting; queueFrame repeats uvc_duplicate_frame per child — expected cost.
	 */
...
static JNINativeMethod methods[] = {
	...
	{ "nativeAddPipeline",				"(JLorg/centennialoss/consolation/uvc/IPipeline;)I", (void *) nativeAddPipeline },
	{ "nativeRemovePipeline",			"(JLorg/centennialoss/consolation/uvc/IPipeline;)I", (void *) nativeRemovePipeline },
...
int register_distribute_pipeline(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"org/centennialoss/consolation/uvc/DistributePipeline",
