...
//								LOGI("chain_frame:%lld,%lld,(%d,%d),actual_bytes=%d", id, dtime, width, height, actual_bytes);
								if (iter != sql_query_oldest_10->end()) {
									/* was 5s — killed replay/live latency; throttle slightly */
									handler_mutex.lock();
									{
										handler_sync.waitRelative(handler_mutex, 3000000LL);
									}
									handler_mutex.unlock();
								}
							}
					...
					try {
						...
						LOGI("exception: failed to delete");
...
int register_sqlite_buffered_pipeline(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"org/centennialoss/consolation/uvc/SQLiteBufferedPipeline",
