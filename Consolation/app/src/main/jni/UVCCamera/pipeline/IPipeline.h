...
public:
	...
	/**
	 * Take ownership of an already-exclusive frame (converted or duplicated upstream),
	 * without queueFrame()'s internal memcpy — when supported.
	 * @return UVC_SUCCESS: pipeline owns *frame*; caller must not recycle it.
	 * UVC_ERROR_NOT_SUPPORTED: caller should queueFrame()+recycle as needed.
	 */
	virtual int tryEnqueueOwning(uvc_frame_t *frame);
};
