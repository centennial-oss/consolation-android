...
/** Low default for latency; overflow drops oldest in add_frame */
#define DEFAULT_MAX_FRAME_NUM 2
...
public:
	...
	/**
	 * Enqueue frame already owned exclusively by this caller (already duplicated
	 * or converted elsewhere). Avoids queueFrame()'s redundant memcpy copy.
	 */
	int queueOwningFrame(uvc_frame_t *owned_frame);
	virtual int tryEnqueueOwning(uvc_frame_t *frame) override;
};
