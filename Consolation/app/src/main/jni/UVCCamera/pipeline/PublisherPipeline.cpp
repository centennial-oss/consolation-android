...
:	AbstractBufferedPipeline(MAX_FRAME_NUM, INIT_FRAME_POOL_SZ, _data_bytes),
	host(addr ? addr : ""),
	subscription_id(_subscription_id ? _subscription_id : ""),
...
int PublisherPipeline::tryEnqueueOwning(uvc_frame_t *frame) {

	(void) frame;
	/* Must dup via queueFrame — chain_frame revisits same pointer afterward */
...
void PublisherPipeline::on_start() {
	...
	if UNLIKELY(host.empty()) {
		LOGW("PublisherPipeline::on_start:no bind address, ZMQ inactive");
		EXIT();
		return;
	}
...
//	LOGD("sub_str=%s,sub_sz=%d", sub_str, sub_sz);
	...
	size_t clipped_len = frame->actual_bytes;
	if (LIKELY(frame->data_bytes) && clipped_len > frame->data_bytes)
		clipped_len = frame->data_bytes;

	publish_header_t header;
	build_header(header, frame);
	header.data_bytes_le = htole32((uint32_t) clipped_len);

	// create payload — never copy past allocated frame.data
	zmq::message_t payload(clipped_len + sizeof(publish_header_t));
	memcpy(payload.data(), &header, sizeof(publish_header_t));
	memcpy(((uint8_t *)payload.data()) + sizeof(publish_header_t), frame->data, clipped_len);
/*
...
static ID_TYPE nativeCreate(JNIEnv *env, jobject thiz,
	...
	const char *c_addr = publisher_addr_str
		? env->GetStringUTFChars(publisher_addr_str, JNI_FALSE) : NULL;
	const char *c_sub_id = subscription_id_str
		? env->GetStringUTFChars(subscription_id_str, JNI_FALSE) : NULL;
	PublisherPipeline *pipeline = new PublisherPipeline(DEFAULT_FRAME_SZ, c_addr, c_sub_id);
	if (publisher_addr_str)
...
static JNINativeMethod methods_publisher_pipeline[] = {
	...
	{ "nativeSetPipeline",	"(JLorg/centennialoss/consolation/uvc/IPipeline;)I", (void *) nativeSetPipeline },
...
int register_publisher_pipeline(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"org/centennialoss/consolation/uvc/PublisherPipeline",
