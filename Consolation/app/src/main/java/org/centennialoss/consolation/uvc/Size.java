...
public class Size implements Parcelable {
	//
	/**
	 * Native {@code uvc_raw_format_t} value; used primarily with libuvc.
	 * 9999 is still image
	 */
	public int type;
	/**
	 * Native {@code raw_frame_t} value (androUSB path);
	 * not used by libuvc.
	 */
	...
	// From here down: filled by #updateFrameRate from frameIntervalType and intervals
	public float[] fps;
	private String frameRates;

	/**
	 * Constructor.
	 * @param _type native {@code raw_format_t}; 9999 means still image
	 * @param _frame_type native {@code raw_frame_t} value
	 * @param _index
	 * @param _width
	 * @param _height
	 */
	...
	private Size(final Parcel source) {
		// Read order must mirror writeToParcel
		type = source.readInt();
	...
	public void setCurrentFrameRate(final float frameRate) {
		// Pick the nearest supported frame rate (at or below the requested value)
		int index = -1;
		final int n = fps != null ? fps.length : 0;
		for (int i = 0; i < n; i++) {
			if (fps[i] <= frameRate) {
	...
	public void updateFrameRate() {
		...
		if (n > 0) {
			...
			try {
				...
				// ignore: min/max can be zero on bad descriptors
				fps = null;
			}
		}
