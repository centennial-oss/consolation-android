...
import org.centennialoss.consolation.BuildConfig;
...
/*		Log.i(TAG, String.format("vendorId=0x%04x,productId=0x%04x,class=0x%02x,subclass=0x%02x,protocol=0x%02x",
	...
	/**
	 * Builds a list of {@link DeviceFilter}s from the given XML resource.
	 * @param context
	...
	public static List<DeviceFilter> getDeviceFilters(final Context context, final int deviceFilterXmlId) {
		...
		try {
			...
			if (BuildConfig.DEBUG) Log.d(TAG, "XmlPullParserException", e);
		} catch (final IOException e) {
			if (BuildConfig.DEBUG) Log.d(TAG, "IOException", e);
		}
...
/*	public void write(XmlSerializer serializer) throws IOException {
	...
	/**
	 * Returns whether the given class, subclass, and protocol match this filter.
	 * The {@code mExclude} flag must be checked separately via {@link #isExclude(UsbDevice)} or equivalent.
	 * @param clasz
	 * @param subclass
	 * @param protocol
	 * @return
	...
	/**
	 * Returns whether {@code device} matches this filter (vendor/product and class hierarchy).
	 * The {@code mExclude} flag must be checked separately via {@link #isExclude(UsbDevice)} or equivalent.
	 * @param device
	 * @return
	 */
...
/*		if (mManufacturerName != null && device.getManufacturerName() == null)
	...
	/**
	 * Returns true when {@code device} matches this filter and {@code mExclude} is true.
	 * @param device
	 * @return
	 */
	...
	/**
	 * Returns whether {@code f} matches this filter's fields (including {@code isExclude}, IDs, names, and class triplet).
	 * May overlap with {@link #equals(Object)} for fully specified filters.
	 * @param f
	 * @return
	 */
